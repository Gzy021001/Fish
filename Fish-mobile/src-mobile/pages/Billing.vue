<template>
  <section class="p-4 space-y-4">
    ...
    <div class="rounded-2xl bg-white/90 p-4 shadow-sm space-y-3">
      <div class="flex items-center gap-2">
        <input
          ...
          class="flex-1 rounded-2xl border border-[#d8c1a0] bg-white px-4 py-3 text-sm text-[#5c4033] outline-none"
          placeholder="搜索品种名称"
          @keydown.enter="applyFilters"
        />
        <button
          class="shrink-0 rounded-2xl border border-[#d8c1a0] bg-white px-3 py-3 text-xs text-[#8a6b4a] active:opacity-70"
          @click="showFilters = !showFilters"
        >
          <svg class="h-4 w-4 transition-transform" :class="{ 'rotate-180': showFilters }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <Transition name="collapse">
        <div v-if="showFilters" class="space-y-3 overflow-hidden">
    ...
    <div class="rounded-2xl bg-white/90 px-4 py-3 shadow-sm">
      ...
      <div class="mt-3 border-t border-[#efe1c8] pt-3 text-xs text-[#8a6b4a]">
    ...
    <div v-else class="space-y-2">
      ...
      <div
        v-if="loadingMore"
        class="rounded-2xl bg-white/90 p-4 text-center text-sm text-[#8a6b4a] shadow-sm"
      >
        加载中...
      </div>
      <div
        v-else-if="!hasMore && bills.length > 0"
        class="rounded-2xl bg-white/90 p-4 text-center text-xs text-[#8a6b4a] shadow-sm"
      >
已无更多数据
      </div>
      <div ref="loadSentinel" class="h-1"></div>
    </div>
    ...
    <Transition name="fade">
      <button
        v-if="bills.length > 0"
        class="fixed bottom-24 left-6 flex h-10 w-10 items-center justify-center rounded-full border border-[#d8c1a0] bg-white/90 text-[#8b6914] shadow-md active:opacity-70 z-50"
        @click="scrollToTop"
      >
