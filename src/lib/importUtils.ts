...
export function parseExcelDate(raw: unknown): string | null {
  ...
  if (match) { return match[1] + "-" + String(parseInt(match[2])).padStart(2,"0") + "-" + String(parseInt(match[3])).padStart(2,"0") }
  return null
}

export function parseSheetDate(sheetName: string, year?: number): string | null {
  const y = year || new Date().getFullYear()
  var Y = String.fromCharCode(0x6708)
  var R = String.fromCharCode(0x65E5)
  var yi = sheetName.indexOf(Y)
  var ri = sheetName.indexOf(R)
  if (yi === -1 || ri === -1) return null
  var month = sheetName.substring(0, yi)
  var day = sheetName.substring(yi + 1, ri)
  return y + "-" + String(parseInt(month)).padStart(2,"0") + "-" + String(parseInt(day)).padStart(2,"0")
}
