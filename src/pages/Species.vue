      ...
      <table class="w-full text-left border-collapse whitespace-nowrap">
        ...
        <tbody class="flex flex-col relative">
          ...
          >
            ...
            <td
              class="px-4 py-1.5 font-mono text-dunhuang-red flex-1 flex items-center"
            >
              ¥ {{ (sp.default_price || 0).toFixed(2) }}
            </td>
        ...
        <form @submit.prevent="addSpecies" class="space-y-5 relative z-10">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <div class="relative">
                <div
                  class="absolute left-0 top-0 bottom-0 px-3 flex items-center justify-center text-dunhuang-blue font-mono text-sm border-r border-dunhuang-yellow/30 bg-dunhuang-yellow/10 rounded-l-lg"
                >
                  ¥
                </div>
                <input
                  ...
                  "
                  class="w-full bg-dunhuang-bg border border-dunhuang-yellow/50 rounded-lg py-3 pl-10 pr-4 text-center focus:ring-2 focus:ring-dunhuang-blue outline-none font-mono text-sm transition-shadow [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                />
              </div>
            </div>
          ...
          <div>
            ...
            <div class="flex items-start gap-5">
              ...
              <div class="flex-1 text-xs text-dunhuang-text/50 space-y-1 pt-1">
                <p>支持 JPG、PNG 格式</p>
                <p>建议尺寸 200×200 像素</p>
                <button
                  v-if="previewUrl"
    ...
    >
      ...
      >
        ...
        <div v-else class="flex-1 flex flex-col min-h-0">
          ...
          <div
            class="flex-1 overflow-auto border border-dunhuang-yellow/30 rounded-lg mb-4 min-h-0"
          >
            <table
            ...
            >
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-4 py-2">{{ row.default_unit }}</td>
                  <td class="px-4 py-2">
                    <button
                      v-if="row.exists"
...
const downloadSpeciesTemplate = () => {
  const templateData = [
    { 品种: "草鱼", "单价 (¥)": 12.5, 默认单位: "公斤" },
    { 品种: "鲈鱼", "单价 (¥)": 25.0, 默认单位: "条" },
  ];
  const worksheet = XLSX.utils.json_to_sheet(templateData);
  worksheet["!cols"] = [{ wch: 15 }, { wch: 12 }, { wch: 12 }];
...
const confirmImportSpecies = async () => {
  ...
  try {
    ...
    const parts = [`成功导入 ${created + updated} 条品种。`];
    if (created > 0) parts.push(`新建 ${created} 条`);
    if (updated > 0) parts.push(`覆盖 ${updated} 条`);
